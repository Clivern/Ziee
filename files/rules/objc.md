...
- A category on a class defined in this repository implements a selector that the original class, a superclass, or another category on the same class also implements, so which implementation runs is undefined; confirm the collision from the diff before reporting it
- A category on a framework class implements a selector that the framework itself already defines, which Apple documents as undefined behavior; treat the override itself as the reportable signal
